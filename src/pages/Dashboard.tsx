    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        <Card className="border-l-4 border-l-primary">
          ...
          <p className={`text-2xl font-bold ${s.total_balance >= 0 ? 'text-secondary' : 'text-accent'}`}>₹{s.total_balance.toLocaleString()}</p>
        </Card>
        <Card className="border-l-4 border-l-secondary">
          <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Total Income</p>
          <p className="text-2xl font-bold text-secondary">₹{s.total_income.toLocaleString()}</p>
        </Card>
        <Card className="border-l-4 border-l-accent">
          <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Total Expense</p>
          <p className="text-2xl font-bold text-accent">₹{s.total_expense.toLocaleString()}</p>
        </Card>
        <Card>
          <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Today</p>
          <p className="text-2xl font-bold text-slate-800 dark:text-white">₹{s.todays_expense.toLocaleString()}</p>
        </Card>
        <Card>
          <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Monthly</p>
          <p className="text-2xl font-bold text-slate-800 dark:text-white">₹{s.monthly_expense.toLocaleString()}</p>
        </Card>
      </div>
      ...
      <Card>
        <h2 className="mb-4 text-lg font-semibold text-slate-800 dark:text-white">Recent transactions</h2>
